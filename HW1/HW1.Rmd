...
for(i in 1:length(muValues))
{
  testStat<-0
  #for(j in ncol(countData))
  #{
   # testStat <- (testStat + (((countData[i,j] - muValues[i]*countLanes[j])^2)/(muValues[i]*countLanes[j])))
  #}
  testStat <- sum((countData[i,] - muValues[i]*countLanes)^2/(muValues[i]*countLanes))
  #print(testStat)
  #print(1-pchisq(testStat,3))
  pValues<-c(pValues,(1-pchisq(testStat,3)))
}
qqplot(pValues,runif(10000),plot.it=TRUE, xlab = "P-Values",main = "QQ Plot")
ks.test(pValues,"punif",0,1)
```

We used the ks.test to check whether the p-values follows that U(0,1) or not. As you can see, the P-value is significant and from that we can reject the null hypothesis which means that the P-Values do not follow U(0,1).

![Question 4](https://raw.githubusercontent.com/ashishjain1988/BCB570/master/HW2/Question2.jpg)
